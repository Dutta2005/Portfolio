...
export default function About() {
  ...
  const skillCategories = [
    ...
    },
    {
      ...
      skills: [
        { name: "LangChain", img: langchainLogo },
        { name: "LangGraph", img: langchainLogo },
        { name: "RAG", lucideIcon: <Search size={28} className="text-amber-400" /> },
        { name: "Agentic AI", lucideIcon: <Bot size={28} className="text-violet-400" /> },
        { name: "OpenAI SDK", img: openaiLogo },
        { name: "MCP", img: mcpLogo },
        { name: "Pinecone", img: pineconeLogo }
      ]
    },
  ];
          ...
          >
            ...
            >
              {skillCategories.map((category, index) => (
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-full transition-all duration-300 flex items-center gap-2 ${activeCategory === category.name
            ...
            >
              {skillCategories.map((category, index) => (
                <motion.div
                  key={index}
                  className={`transition-opacity duration-300 ${activeCategory === category.name ? "block" : "hidden"
                    }`}
                  initial="hidden"
                  ...
                  >
                    {category.skills.map((skill, skillIndex) => (
                      ...
                      >
                        ...
                        >
                          {skill.img ? (
                            <img src={skill.img} alt={skill.name} className="w-8 h-8 md:w-10 md:h-10 object-contain" />
                          ) : skill.lucideIcon ? (
                            skill.lucideIcon
                          ) : skill.icon ? (
